...
Для этого в netlab есть отдельный раздел в топологии [vlaledate](https://netlab.tools/topology/validate)
...
Можно составлять и более сложные тесты, и проверять вывод show комманд оборудования

Давйте проверим, что для всех соседей, указанных в топологии, была установлена BGP сессия (пример из документации, только больше чем с 2мя соседями не работало, пришлось немного переписать)
...
Здесь bgp.neighbors можно посмотреть в файле netlab.snapshot.yml. В этом файле указываются все данные по топологии. Если в show на выходе получается валиждный json, то результат помещается в переменную result.&#x20;
